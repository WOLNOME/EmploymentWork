#include "ItemTutorialCollectible.h"
#include <TextureManager.h>
#include <Object3dManager.h>
#include <CombinedParticleManager.h>

//アプリケーション
#include <application/object/character/item/tutorialCollectible/collision/ItemTutorialCollectibleCollider.h>

using namespace Norm;

void ItemTutorialCollectible::Initialize() {
	//ベースキャラクターの初期化
	...
	//オブジェクトを生成・初期化
	textureHandle_ = TextureManager::GetInstance()->LoadTexture("yellow.png");
	object3d_->Initialize(ShapeTag{}, Object3dManager::GetInstance()->GenerateName("Item_TutorialCollectible"), Shape::ShapeKind::kCube);
	...
	//当たり判定の生成・初期化
	collider_ = std::make_unique<ItemTutorialCollectibleCollider>(this);
...
void ItemTutorialCollectible::Update() {
	//ベースキャラクターの更新
...
void ItemTutorialCollectible::DebugWithImGui() {
#ifdef _DEBUG
	//ベースキャラクターのデバッグ処理
...
void ItemTutorialCollectible::Spawn(const Vector3& _initPos) {
	//初期座標を保存
	...
	isUp_ = true;
	//コライダーの属性を決める
	collider_->SetCollisionAttribute(CollisionAttribute::Item_TutorialCollectible);
	//アクティブ状態にする
	SetState(State::kActive);

}

void ItemTutorialCollectible::UntilDeathProcess() {
	//共通処理
	{
		// タイマー更新
...
void ItemTutorialCollectible::UpdateParticle() {
	TransformEuler transform = idleParticle_->GetBaseTransform();
