...
private://メンバ変数
	//移動
	const float searchPDistanceMove_ = 300.0f;		//プレイヤー探索距離
	const float speed_ = 4.0f;						//移動スピード
	const float maxSpeed_ = 30.0f;					//最大移動スピード

	//回転
	const float searchPDistanceRotate_ = 300.0f;	//プレイヤー探索距離
	const float rotateSpeed_ = 1.0f / 2.5f * pi;	//回転スピード

	//攻撃
	const float searchPDistanceAttack_ = 150.0f;	//プレイヤー探索距離
	const float attackCoolTime_ = 1.5f;				//攻撃のクールタイム
	float attackCoolTimer_ = 0.0f;					//計測用タイマー
	bool isEnemyAttacked_ = false;

	//HP
	const int maxHP_ = 200;		//最大HP
