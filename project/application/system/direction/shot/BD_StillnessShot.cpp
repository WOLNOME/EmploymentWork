...
BD_StillnessShot::BD_StillnessShot(int _index, Norm::TransformEuler _startCameraTransform, Norm::TransformEuler _endCameraTransform, float _duration, Norm::BlackBoard* _blackBoard) : IShotBase(_index, _startCameraTransform, _endCameraTransform, _duration, _blackBoard) {
	//パラメーターの初期化
	param_ = JsonUtil::GetJsonData("Resources/parameters/bossDestroySystem");

}

BD_StillnessShot::~BD_StillnessShot() {
}

void BD_StillnessShot::Initialize() {
	//ブラックボードから必要な情報を取得
...
void BD_StillnessShot::Finalize() {
	//ブラックボードから必要な情報を取得
	BaseCamera* camera = blackBoard_->GetValue<BaseCamera*>("Camera");
